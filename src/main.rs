...
fn draw_pic(x: i32, y: i32) -> String {
    ...
    for j in (0..y).rev() {
        for i in 0..x {
            ...
            let r: Ray = Ray::new(origin, lower_left_corner + horizontal.add_diff(u) + vertical.add_diff(v));
...
fn color(r: Ray) -> Vec3 {
    ...
    if t > 0.0 {
        ...
        return Vec3::new(new_v.x() + 1.0, new_v.y() + 1.0, new_v.z() + 1.0).add_diff(0.5);
    }

    let unit_direction = Vec3::unit_vector(r.get_direction());
    let t: f64 = 0.5 * (unit_direction.y() + 1.0);

    Vec3::new_by_val(1.0).add_diff(1.0 - t) + Vec3::new(0.2, 0.7, 1.0).add_diff(t)
}
